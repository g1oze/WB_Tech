...
WITH t AS
	(SELECT 
		*, 
		LEAD(query, 1) OVER(PARTITION BY userid, devicetype ORDER BY ts ASC) AS next_query,
		-- Сюда помещаю время следующего запроса данного пользователя с данного устройства.
		LEAD(ts, 1) OVER(PARTITION BY userid, devicetype ORDER BY ts ASC) AS next_query_ts
	FROM query 
	ORDER BY searchid)
